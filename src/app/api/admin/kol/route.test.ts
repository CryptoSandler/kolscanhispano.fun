...
const PROVENANCE =
  "Solana Tracker KOL leaderboard https://data.solanatracker.io/v2/pnl/leaderboard/kols, read 2026-09-01";
...
function request(body: unknown, token: string | null = TOKEN): Request {
  return new Request("https://kolscanhispano.fun/api/admin/kol", {
    ...
    headers: {
      ...
      /*
        Una IP distinta por petición. Las rutas de admin corren el limitador
        desde el 2026-09-07 (`admin-limits.test.ts`), y un archivo que llama
        veinte veces desde `203.0.113.7` se pasa del bucket `admin-write` y
        empieza a recibir 429 — un fallo que no dice nada sobre lo que el caso
        quería medir. Es lo mismo que hace `rate-limit-wiring.test.ts`.
      */
...
describe("1. the gate", () => {
  it("refuses with no token, a wrong token, an empty token and the wrong scheme", async () => {
    ...
    for (const header of headers) {
      ...
      expect(
        response.status,
        `header: ${header === null ? "(none)" : header}`,
      ).toBe(401);
...
describe("2-6. what a wallet has to be", () => {
  it("2. refuses an address of the wrong shape for its chain", async () => {
    const response = await POST(
      request(oneWallet({ address: inventEvmAddress() })),
    );
  ...
  it("3. refuses a chain with no live ingestion, and says which are live", async () => {
    const response = await POST(
      request({
        handle: "ejemplo",
        wallets: [{ address: inventEvmAddress(), chain: "ethereum" }],
      }),
    );
    expect(response.status).toBe(400);
    /*
  ...
  it("3b. tells an unknown chain apart from an inactive one", async () => {
    ...
    const response = await POST(
      request({
        handle: "ejemplo",
        wallets: [{ address: inventAddress(), chain: "polygon" }],
      }),
    );
  ...
  it("4. refuses an address already held by another KOL, without naming them", async () => {
    ...
    expect(
      await query("SELECT id FROM kol WHERE slug = 'segundo'"),
    ).toHaveLength(0);
  });
  ...
  it("refuses a KOL with no wallets at all", async () => {
    expect(
      await (await POST(request({ handle: "ejemplo", wallets: [] }))).json(),
    ).toEqual({
      error: "no_wallets",
    });
  });
...
describe("7. what a successful creation leaves", () => {
  ...
  it("creates the KOL pending when asked, and leaves approved_at null", async () => {
    ...
    const [kol] = await query<{ status: string; approved_at: Date | null }>(
      "SELECT status, approved_at FROM kol WHERE id = $1",
      [body.kolId],
    );
  ...
  it("stores the provenance of a staged candidate in the audit trail", async () => {
    ...
    const [row] = await query<{ after: { provenance: string } }>(
      "SELECT after FROM audit_log WHERE target_id = $1 AND action = 'kol.create'",
      [kolId],
    );
  ...
  it("still creates approved when no status is sent", async () => {
    ...
    const { kolId, status } = (await response.json()) as {
      kolId: string;
      status: string;
    };
    expect(status).toBe("approved");

    const [kol] = await query<{ status: string }>(
      "SELECT status FROM kol WHERE id = $1",
      [kolId],
    );
  ...
  it("creates the KOL approved, with its wallets private by default", async () => {
    ...
    expect(body.wallets).toEqual([
      { id: expect.any(String), chain: "solana", isPublic: false },
    ]);

    const [kol] = await query<{
      status: string;
      approved_at: Date | null;
      x_handle: string;
    }>("SELECT status, approved_at, x_handle FROM kol WHERE id = $1", [
      body.kolId,
    ]);
    expect(kol.status).toBe("approved");
    ...
    const [wallet] = await query<{
      ...
      "SELECT address_enc, address_hmac, is_public FROM kol_wallet WHERE kol_id = $1",
      [body.kolId],
    );
  ...
  it("writes no audit row for a refused creation", async () => {
    await POST(
      request({
        handle: "ejemplo",
        wallets: [{ address: "nope", chain: "solana" }],
      }),
    );
